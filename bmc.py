...
def _get_collection_path(ip, username, password, collection):
    """Get first member @odata.id from a Redfish collection — works for any brand."""
    try:
        r = requests.get(
            f"https://{ip}/redfish/v1/{collection}/",
...
def get_server_info(server_key=None, ip=None, username=None, password=None):
    ...
    try:
        ...
        systems_path = _get_collection_path(target["ip"], target["username"], target["password"], "Systems")
        if not systems_path:
            return {"error": "Could not discover Redfish Systems path — check BMC connectivity"}
        response = requests.get(
            f"https://{target['ip']}{systems_path}",
            auth=auth,
            verify=False,
            timeout=10
        )
...
def power_action(server_key, action):
    ...
    try:
        systems_path = _get_collection_path(server["ip"], server["username"], server["password"], "Systems")
        if not systems_path:
            return {"error": "Could not discover Redfish Systems path"}
        response = requests.post(
            f"https://{server['ip']}{systems_path}/Actions/ComputerSystem.Reset",
...
if __name__ == "__main__":
    print("Testing BMC from database...")
    servers = get_bmc_servers()
    if not servers:
        print("No BMC devices in database. Add via Settings page.")
    else:
